...
Pour chaque tweet dans la base de donnée, le script recherche le nombre de favoris et le nombre de retweets
et met à jour la BDD en conséquence.
Cette collecte est faite séparemment de la collecte des tweets car le nombre de favoris et de rt
évolue dans les jours suivant la diffusion du tweet
"""
...
def main():
    tweets_to_process = session.query(TWEET).filter(TWEET.nb_rt==None)
    while tweets_to_process.count() > 0:
        lg.info(f"Searching influence data. Tweets remaining : {tweets_to_process.count()}")
