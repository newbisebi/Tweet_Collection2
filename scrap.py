...
def parse_arguments():
    ...
    parser.add_argument("-o", "--older", help="""Optional. Use to get tweets older than those already stored in db""", action="store_true")
...
def run_keyword(keyword, older):
    ...
    while keep_looking:
        i += 1
        try:
            lg.info(f"Searching for keyword {keyword.valeur}  ; loop number {i}")
            if older is True:
                ...
                older = True #then we search tweet oldest than those just collected
            min_id = manage_result(res, keyword)
            if not min_id : keep_looking = False


        except TwythonRateLimitError:
            lg.warning("Twitter limit reached. Wait 15 minutes before moving to next keyword")
            # time.sleep(900)
            break
...
def manage_result(res, keyword):
    ...
    if res['statuses']: #case where there are some results to write in database
        ...
        lg.info("No more results for keyword '%s' ; moving to next keyword", keyword.valeur)
        return None
...
def main(older=True):
    keywords = session.query(KEYWORD).filter(KEYWORD.active==True).order_by(KEYWORD.nb_rech).all()
